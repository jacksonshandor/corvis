...
class RatBot
  attr_reader :name, :level, :exp_gain, :exp_to_next_level, :stats

  def initialize(name, level, exp_gain, difficulty = 1)
    name_list = ["Rat", "Mouse", "Rodent", "Vermin", "Pest", "Critter"]
    @name = name_list.sample + " Bot"
    @stats = { hp: rand(10 * difficulty) + 1, attack: rand(5 * difficulty) + 1, defense: rand(3 * difficulty) + 1 }
    @level = level
    @exp_gain = exp_gain
  end
...
class ChatBot
  ...
  def go_to_combat(ratbot)
    ...
    loop do
      ...
      decision_ratbot = ["attack", "defend"].sample
      ...
      if ratbot.stats[:hp] <= 0
        response += "#{ratbot.name} has been defeated!"
        exp_gain(ratbot.exp_gain)
